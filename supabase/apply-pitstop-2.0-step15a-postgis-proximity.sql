-- PitStop 2.0 — Step 15a: PostGIS proximity search for nearby wash branches & maintenance shops
-- Run in Supabase SQL Editor after step 14d.

create extension if not exists postgis;

-- ---------------------------------------------------------------------------
-- Geography columns (derived from existing latitude / longitude)
-- ---------------------------------------------------------------------------
...
-- Back-fill branch rows that inherit shop coordinates when branch lat/lng is null
...
-- Keep geog in sync when owners update pins
...
-- ---------------------------------------------------------------------------
-- find_nearby_listings — server-side ST_DWithin + ST_Distance (km)
-- p_type: 'wash' | 'maintenance'
-- When p_lat / p_lng are null, returns active listings without distance filtering.
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- wash: one row per active branch; shops without branches use shop pin
  return query
  with branch_rows as (
    select
      s.id as shop_id,
      ...
      coalesce(b.name, s.name) as listing_name,
      coalesce(b.name_ar, s.name_ar) as listing_name_ar,
...
comment on function public.find_nearby_listings(numeric, numeric, text, numeric, int) is
  'PostGIS proximity search for active wash branches or maintenance shops; distance_km via ST_Distance.';
