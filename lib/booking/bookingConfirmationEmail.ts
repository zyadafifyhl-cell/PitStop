...
export function buildBookingConfirmationHtml(input: BookingConfirmationEmailInput): string {
  ...
  const serviceRows = services
    .map(
      (line) =>
        `<tr><td style="padding:8px 12px;border-bottom:1px solid #E2E8F0;">${escapeHtml(line)}</td></tr>`,
    )
    .join('');
      ...
      <table role="presentation" width="100%" style="max-width:560px;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #E2E8F0;">
        ...
        <tr>
          <td style="padding:24px;">
            ...
            <table role="presentation" width="100%" style="border:1px solid #E2E8F0;border-radius:12px;margin-bottom:16px;">
              <tr><td style="padding:12px 14px;background:#F8FAFC;font-weight:800;color:#111827;">
                ${isAr ? 'تفاصيل الموعد' : 'Appointment details'}
              </td></tr>
              <tr><td style="padding:10px 14px;color:#0F172A;font-size:14px;">
            ...
            <table role="presentation" width="100%" style="border:1px solid #E2E8F0;border-radius:12px;margin-bottom:16px;">
              ...
              ${serviceRows}
              <tr><td style="padding:12px 14px;font-weight:800;color:#0F172A;font-size:16px;">
                ${isAr ? 'الإجمالي:' : 'Total:'} ${escapeHtml(formatEgp(money.servicePriceEgp, locale))}
              </td></tr>
            </table>
            ${booking.customerNotes ? `<p style="margin:0 0 12px;color:#64748B;font-size:13px;line-height:1.5;"><strong>${isAr ? 'ملاحظات:' : 'Notes:'}</strong> ${escapeHtml(booking.customerNotes)}</p>` : ''}
        ...
        <tr>
          <td style="padding:16px 24px;background:#F8FAFC;text-align:center;color:#94A3B8;font-size:11px;">
