import type { StoreCartItem, StoreCartShopGroup, StoreProduct } from '@/lib/store/types';
...
export function cartSubtotal(items: StoreCartItem[]): number {
  return items.reduce(
    (sum, row) => sum + (row.product?.salePrice ?? row.product?.price ?? 0) * row.quantity,
    0,
  );
}
...
export function groupCartItemsByShop(
  ...
  const groups = new Map<string, StoreCartShopGroup>();
  for (const item of items) {
    const shopId = item.product?.shopId ?? 'unknown';
    const shop = shopId !== 'unknown' ? getShopById(shopId) : undefined;
    const shopName =
      shop == null
        ? shopId
        : locale === 'ar'
    ...
    if (existing) {
      ...
      existing.subtotal += (item.product?.salePrice ?? item.product?.price ?? 0) * item.quantity;
      continue;
    }
    groups.set(shopId, {
      shopId,
