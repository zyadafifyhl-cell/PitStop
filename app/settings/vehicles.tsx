...
const emptyForm = (): VehicleForm => ({ label: '', makeModel: '', color: '', plate: '' });
  ...
  async function onSave() {
    ...
    if (editingId) {
      await updateCustomerVehicle(customer.id, editingId, {
        label: form.label.trim() || makeModel,
        makeModel,
        color: form.color.trim() || undefined,
        plate: form.plate.trim() || undefined,
      });
    } else {
      await addCustomerVehicle(customer.id, {
      ...
      {vehicles.length === 0 ? (
        ...
        vehicles.map((vehicle) => (
          <View key={vehicle.id} style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border }]}>
            <Text style={[styles.cardTitle, { color: theme.text }]}>{vehicle.label}</Text>
            <Text style={[styles.cardMeta, { color: theme.textMuted }]}>{vehicle.makeModel}</Text>
            {vehicle.color ? (
              <Text style={[styles.cardMeta, { color: theme.textMuted }]}>{vehicle.color}</Text>
            ) : null}
            {vehicle.plate ? (
              <Text style={[styles.cardMeta, { color: theme.textMuted }]}>{vehicle.plate}</Text>
            ) : null}
        ...
        <View style={styles.modalBackdrop}>
          <View style={[styles.modalCard, { backgroundColor: theme.card, borderColor: theme.border }]}>
            ...
            <TextInput
              ...
              placeholder={t('add_nickname_label')}
            ...
            <TextInput
              value={form.plate}
              onChangeText={(plate) => setForm((prev) => ({ ...prev, plate }))}
              placeholder={t('book_car_type_label')}
              placeholderTextColor={theme.textDim}
              style={[styles.input, { color: theme.text, borderColor: theme.border, backgroundColor: theme.bgElevated }]}
            />
